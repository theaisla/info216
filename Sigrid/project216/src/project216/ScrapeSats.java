...
public class ScrapeSats implements IScraper {
	...
	public void extractDataFromPage() throws IOException{
		...
		for (Element e: document.select("table.table.booking-table.booking-table-mobile.visible-sm.visible-xs.hidden-print.ng-scope tr")){ //".ng-scope"
			theDay =  e.select("table booking-table table-expandable-striped visible-md visible-lg hidden-print ng-scope").text();
		...
		//	System.out.println(title);
			if (createResource(title)){
				...
				//System.out.println(e.select("table booking-table table-expandable-striped visible-md visible-lg hidden-print ng-scope").text());
				//plukker ut bugs
				...
				if (timeAndLocationDetails.length > 4){
					...
					//allResources.get(numResources-1).addProperty(instruktoer, instructor).addProperty(varighet, duration).addProperty(treningssenter, center).addProperty(starttid, timeInStringFormat);
					System.out.printf("%50s %10s %5s %15s %15s %30s \n", title, timeInStringFormat, duration, location, center, instructor  );
				}
				else {
					theDay = e.text();
					System.out.println( theDay);
