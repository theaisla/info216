...
function createWHERE() {
  ...
  for (x in names){
    if(names[x] == "dayOfWeek"){
      res.push("?timer a:dayOfWeek ?value .  FILTER(?value IN(" + listDay + "))");
    }
    if(names[x] == "startTime"){
    ...
    if(names[x] == "sameAs"){
      ...
      res.push("?timer a:sameAs ?value2 .  FILTER(?value2 IN(" + listTitle2 + "))");
    }
    if(names[x] == "duration"){
...
/**
  ...
  * http://semapps.blogspot.no/2012/05/sparql-query-from-javascript.html
  * Author: Mark Wallace
 */
...
// Define endpoint. Fuseki, make sure dataset is called ds !!!
  ...
  function myCallback(str) {
    ...
    result += "</table>" ;
     ...
     if(jsonObj.results.bindings.length == 0) {
       document.getElementById("demo").innerHTML = "<h3>Kunne ikke finne noen resultater med dette søket.</h3>";
     } else {
